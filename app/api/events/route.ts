import { generateSignalFeed } from "@/lib/event-generator";
...
export async function GET() {
  try {
    const { events, quality } = await generateSignalFeed();
    ...
    return Response.json(events, { headers: { "X-SVA-Signal-Quality": JSON.stringify(quality) } });
  } catch (error) {
